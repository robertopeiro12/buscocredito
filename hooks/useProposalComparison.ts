...
export function useProposalComparison({
  ...
  const fetchComparisons = async () => {
    ...
    try {
      ...
      const results: ProposalComparisonItem[] = [];

      // Get unique loanIds from admin proposals
      ...
      // For each loanId, find all proposals to identify the accepted one
      for (let i = 0; i < loanIds.length; i++) {
        const loanId = loanIds[i];

        // Find admin's proposal for this loan
        const adminProposal = rawProposals.find(
          (p) => (p.loanId || p.solicitudId) === loanId
        );
        if (!adminProposal) continue;

        // Query all proposals for this loan
        const proposalsRef = collection(db, "propuestas");
        const q = query(proposalsRef, where("loanId", "==", loanId));
        ...
        // Also try querying by solicitudId for legacy data
        if (!acceptedProposal) {
          const legacyQ = query(proposalsRef, where("solicitudId", "==", loanId));
        ...
        // Calculate differences (admin vs accepted)
        ...
        // Try to get original solicitud amount
        try {
          ...
          // Ignore - use fallback from proposal
        }
